...
def get_data_loaders(batch_size: int = BATCH_SIZE):
    ...
    test_dataset = datasets.ImageFolder(
        TEST_DIR, transform=get_val_transforms(IMG_SIZE)
    )
...
def train_model(args):
    ...
    with mlflow.start_run(
        ...
        cm_fig = plot_confusion_matrix(
            cm, CLASS_NAMES, save_path=MODELS_DIR / "confusion_matrix.png"
        )
        mlflow.log_figure(cm_fig, "confusion_matrix.png")

        # Training history plot
        history_fig = plot_training_history(
            history, save_path=MODELS_DIR / "training_history.png"
        )
