...
from src.inference.predictor import CatDogPredictor  # noqa: E402
from src.utils.config import API_HOST, API_PORT, MODELS_DIR  # noqa: E402
...
REQUEST_COUNT = Counter(
    "cats_dogs_requests_total", "Total number of prediction requests", ["status"]
)
...
PREDICTION_CLASS = Counter(
    "cats_dogs_predictions_total", "Total predictions by class", ["predicted_class"]
)
MODEL_LOADED = Gauge(
    "cats_dogs_model_loaded", "Whether the model is loaded (1) or not (0)"
)
...
async def startup_event():
    ...
    try:
        ...
        if not model_path.exists():
            logger.warning(
                f"Best model not found at {model_path}, trying final_model.pt"
            )
            model_path = MODELS_DIR / "final_model.pt"

        # If model still not found, try to download from Google Drive
        if not model_path.exists():
            logger.info(
                "Model not found locally, attempting to download from Google Drive..."
            )
            try:
                from scripts.download_model import ensure_model_exists
                ...
                raise FileNotFoundError(
                    ...
                    "  1. Place best_model.pt in the models/ directory\n"
                    "  2. Set GDRIVE_FOLDER_ID env var and ensure gdown is installed\n"
                    "  3. Run: python scripts/download_model.py"
                )
